...
subplot(3,2,5)
circ_conv_vec=ifft(fft(x_vec).*fft(y_vec));plot(t_vec,circ_conv_vec,'-o');hold on;
circ_conv_vec1=cconv(x_vec,y_vec,K);plot(t_vec,circ_conv_vec1,'-x')
...
subplot(3,2,6)
circ_corr_vec=ifft(conj(fft(x_vec)).*fft(y_vec));plot(t_vec,circ_corr_vec,'-o');hold on;
